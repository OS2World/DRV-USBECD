...
dataseg
sInfoP db ' setting video probe',13,10
sInfoQ db ' getting video probe',13,10
sInfoR db ' committing video format',13,10
label sInfoS byte

dataseg
sTest0 db ' opening video.raw',13,10
sTest1 db ' closing video.raw',13,10
label sTest2 byte
...
dataseg
SetNegotiate db 021h,01h,00h,01h,01h,00h,1Ah,00h,26 dup(0)
GetSuggested db 0A1h,81h,00h,01h,01h,00h,1Ah,00h,26 dup(0)
SetSuggested db 021h,01h,00h,02h,01h,00h,1Ah,00h,26 dup(0)
...
; open usb video stream device driver
...
; supply negotiate setting
  call DosWrite c,1,offset(sInfoP),sInfoQ-sInfoP,offset(BytesDone)
  call DosWrite c,[fhDevice],offset(SetNegotiate),34,offset(BytesDone)
  call ShowReturnCode
  jnz NotIsoOpenAttempt
; obtain suggested setting
  call DosWrite c,1,offset(sInfoQ),sInfoR-sInfoQ,offset(BytesDone)
  call DosWrite c,[fhDevice],offset(GetSuggested),34,offset(BytesDone)
  call ShowReturnCode
  jnz NotIsoOpenAttempt
; commit suggested setting
  mov ecx,26 ; size suggestion
  mov esi,offset(GetSuggested+8)
  mov edi,offset(SetSuggested+8)
  rep movsb ; copy suggested setting
  call DosWrite c,1,offset(sInfoR),sInfoS-sInfoR,offset(BytesDone)
  call DosWrite c,[fhDevice],offset(SetSuggested),34,offset(BytesDone)
  call ShowReturnCode
  jnz NotIsoOpenAttempt
...
; close usb video stream device driver
...
; parm DeviceDriverName
  ...
  call chr2ddn ; update
  jmp ProcessArguments
label NotDriverName near
; parm bFormatIndex
  cmp [byte(edi)],'x'
  jne ProcessArguments
  call hex2bin ; convert
; override bFormatIndex
  mov [SetNegotiate+10],dl
  jmp ProcessArguments
